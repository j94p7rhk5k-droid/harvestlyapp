...
export function useChat(options: UseChatOptions) {
  ...
  // Track categories created during this chat session (for ID resolution)
      ...
      try {
        switch (action.type) {
          case 'add_category': {
            ...
            // Store the created category ID so subsequent transactions can use it
          ...
          case 'add_transaction': {
            ...
            // Resolve category ID: use provided ID, or look up from recently created, or find in budget
            let resolvedId = categoryId;
            if (!resolvedId || resolvedId === '') {
              // Check recently created categories first
              resolvedId = createdCategoriesRef.current.get(categoryName.toLowerCase());
            }
            if (!resolvedId) {
              // Find in existing budget categories
  ...
  const sendMessage = useCallback(
    async (text: string, files?: File[]) => {
      ...
      try {
        // Build conversation history (exclude welcome message actions/metadata)
    ...
    [effectiveUserId, messages, budgetMonth, month, currency, executeAction],
  );
